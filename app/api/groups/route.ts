...
export async function POST(request: Request) {
  ...
  const statements = [
    ...
    ...people.flatMap(person => person.expenses.flatMap(expense => (expense.splitWith?.length ? expense.splitWith : people.map(member => member.id)).map(participantId => env.DB.prepare("INSERT INTO expense_participants (expense_id, person_id, group_id) VALUES (?, ?, ?)").bind(expense.id, participantId, id)))),
  ];
